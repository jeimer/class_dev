...
def prefilter_swig_dict(d_dict, mode = 'readout', tau_path = None):
    '''
    ...
    mode(string): options are:
        'readout' : deconvolve readout only
        'tau' : deconvole time constants only, if tau_path is not supplied,
        the function attempts to find a best-fit.
        a bit-fit is attempted to be found. 
        'fulltime' : deconvolve readout and time constants, if tau_path
        is not supplied, the functions attempts to find a best-fit.
        'calib' : apply an iv calibration to the d_dict
        'full' : assume the d_dict data is raw, deconvole the readout and the
        detector time constant, and apply an iv calibration to the data. 
    ...
    t_apply = {'readout':deconvolve_reaout,
               'tau': deconvolve_tau,
               'fulltime': deconvolve_r_and_t,
               'calib': calibrate_dict,
               'full': full_prefilter}
