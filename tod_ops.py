...
def win_sinc(bw, fc, n, mode = 'low'):
    '''
    returns the impulse response of a low (hi) Blackman windowed sinc function
    having bw transition width, fc cuttoff, of length n.
    Parameters:
    bw(float): appoximate width of roll-off as fraction of sampling frequency
    fc(float): cuttoff frequency of filter as fraction of sampling frequency
    n(int): length of output array. must be long enough to be consistent with
    specified bw.
    mode(srtring):
     - 'low' - win_sinc will return a low-pass filter
     - 'hi' - win_sinc will return the high-pass filter generated by spectral
       inversion
    Returns:
    h(np.array): the impulse response of a Blackman windowed sinc
    filter
    '''
    m = int(np.ceil( 4. /bw / 2.) * 2)
    s = np.arange(m)
    s[m/2] = 1
    f = np.sin(2 * np.pi* fc * (s - m/2))/(s - m/2) * black_wind(m)
    f[m/2] = 2 * np.pi * fc
    norm = f.sum()
    h = np.zeros(n)
    h[:len(f)] = f
